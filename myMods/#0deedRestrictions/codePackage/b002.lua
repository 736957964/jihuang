...
local Introduce = require("widgets/B002/chh_B002_introduce")


local tab = {
	MODROOT = MODROOT..'json/gift_info.json',
	UI = { ASSET = "images/B002/",PUBLIC = "images/public/" }
}

if TUNING.CHHDATA then TUNING.CHHDATA.B002={} TUNING.CHHDATA.B002 = tab else TUNING.CHHDATA = { B002 = tab } end

--UI部分 所有需要注册的东西都插入在这张表
if not Assets then Assets = { } end
local UI ={ --注册的数据  对应文件夹路径 TUNING.CHHDATA.B002.UI.ASSET
	ASSET = {'background','poker'},
	PUBLIC = {'chh_close'}
}
for index,name in pairs(UI) do
	for k,v in pairs(name) do
		table.insert(Assets,Asset("ATLAS", TUNING.CHHDATA.B002.UI[index]..v..".xml"))
		table.insert(Assets,Asset("IMAGE", TUNING.CHHDATA.B002.UI[index]..v..".tex"))
	end
end
---重复的代码 设置图标等比例变化
