...
local function Hatch(inst)
	...
	local ents = TheSim:FindEntities(pt.x, pt.y, pt.z, SEE_DIST, { "mossling" })
	local findCount = ents ~= nil and #ents or 0
	local mosslingCount = TUNING.MOOSE_EGG_NUM_MOSSLINGS > findCount and (TUNING.MOOSE_EGG_NUM_MOSSLINGS - findCount) or 0

	for i = 1, mosslingCount do
