...
local function LightningStrike(inst)
    ...
    local pos = target:GetPosition() + offset
	TheWorld:PushEvent("ms_sendlightningstrike", pos)
	TheWorld:PushEvent("ms_forceprecipitation", true)
end

-- 制造变异龙卷风
local function SpawnChangeTornado()
	...
	tornado:ListenForEvent("timerdone", ontimerdone)
	tornado:SetStateGraph("SGtornado_moose")
	tornado.task:Cancel()
	tornado:SetDuration(TUNING.MOOSE_TORNADO_TIME)
	return tornado
end
...
-- 大招:春鸭龙卷风
local function SpawnTornado(inst, data, pos, entity, target, owner)
    if inst then
		local target = inst.components.combat.target
		if target then
			local tornado = SpawnChangeTornado()
...
local function SpawnEgg(inst)
	local findEgg = FindEntity(inst, SEE_ITEM_DIST, function(guy)
...
local states=
{
	State{
		...
		onupdate = function(inst, data)
			local target = inst.components.combat.target 
			if inst.tornado_cd and inst and target then           
				if not inst.components.timer:TimerExists("tornado_cd") and inst.components.combat.target:GetPosition():Dist(inst:GetPosition()) > 15 then
					if math.random() < 0.25 then
						inst.sg:GoToState("gust")
                    end
                end
            end
        end,
	...
	State{
        ...
        timeline=
        {
            ...
            TimeEvent(17*FRAMES, function(inst)
				...
				if not inst.components.timer:TimerExists("tornado_cd") then
					inst.components.timer:StartTimer("tornado_cd", inst.tornado_cd or 1)
				end
			end),
            TimeEvent(21*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve_DLC001/creatures/moose/flap") end),
            TimeEvent(27*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve_DLC001/creatures/moose/flap") end),
	...
	State{
		...
		timeline =
		{
			...
			TimeEvent(12*FRAMES, function(inst)
				...
				if inst.components.combat.target and inst.components.combat.target.ShakeCamera then
					inst.components.combat.target:ShakeCamera(CAMERASHAKE.FULL, 0.75, 0.01, 2, SHAKE_DIST)
				end
			end),
			TimeEvent(15*FRAMES, function(inst)
				DisarmTarget(inst, inst.components.combat.target)
				if inst.tornado_cd and inst.components.entitytracker:GetEntity("egg") then
            		LightningStrike(inst)
                end
			end),
			TimeEvent(29*FRAMES, function(inst)
				PlayFootstep(inst)
				if inst.tornado_cd and not inst.components.entitytracker:GetEntity("egg") then
					SpawnEgg(inst)
				end
			end),
		},
		...
		events=
		{
			EventHandler("animqueueover", function(inst)
				if inst.tornado_cd and not inst.components.timer:TimerExists("tornado_cd") then
					inst.sg:GoToState("gust")
				else
					inst.sg:GoToState("idle")
				end
