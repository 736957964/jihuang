...
local events=
{
    EventHandler("attacked", function(inst) 
		--if not inst.components.health:IsDead() and not inst.sg:HasStateTag("nointerrupt") and not inst.sg:HasStateTag("attack") then inst.sg:GoToState("hit") end 
	end),
    EventHandler("death", function(inst) inst.sg:GoToState("death") end),
    EventHandler("doattack", function(inst) 
		if not inst.components.health:IsDead() then 
			local atkran=math.random()
			if atkran<.5 then
	...
	State{
        ...
        timeline=
        {
            ...
            TimeEvent(18*FRAMES, function(inst) 
				...
				if tar and tar:HasTag("player") and not tar.components.health:IsDead() then
					local dist = inst:GetPosition():Dist( tar:GetPosition() )
					if dist and dist< (inst.components.combat.hitrange or 3)+1 then
						inst.components.talker:Say("逮到你了吧!")
						inst:DoTaskInTime(0,function()
							if not tar.sg:HasStateTag("zg_bepicked") then
								tar:PushEvent("zg_buff_bepicked",{attacker=inst})
							end
						end)
					else
						inst.components.talker:Say("哇擦!没逮到")
					end
				end
			end),
        },
...
return StateGraph("zg_krampus", states, events, "idle", actionhandlers)
