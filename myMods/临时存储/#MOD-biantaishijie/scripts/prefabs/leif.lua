...
local function ontimerdone(inst, data)
    if data.name == "TreeDance" then
        inst.TreeDance = true
    end
end

local function Photosynthesis(inst)
	if TheWorld.state.isday and not TheWorld.state.iscaveday then
	inst.components.health:DoDelta(10,false,"leif")
else
	return
	end
end
...
local function common_fn(build)
    ...
    ------------------
	---------改
	inst:DoPeriodicTask(1, Photosynthesis)
	...
	---------改
	inst.components.combat:SetRange(3, 4.5)
    inst.components.combat:SetAreaDamage(4, 0.8)
    inst.components.combat.notags = {"tree", "leif"}
	inst:AddComponent("groundpounder")
    inst.components.groundpounder.destroyer = true
    inst.components.groundpounder.damageRings = 1
    inst.components.groundpounder.groundpounddamagemult = .10
    inst.components.groundpounder.destructionRings = 1
    inst.components.groundpounder.numRings = 2
    inst.components.groundpounder.noTags = {"tree", "leif"}
