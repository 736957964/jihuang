...
local function CalcChompSpeed(inst, target)
    ...
    if distsq > 0 then
        ...
        if dist > 0 then
            return math.min(6, dist) / (10 * FRAMES)
        end
    end
    return 0
end
...
local function ClearRecentlyBounced(inst, other) --清除
...
local function BounceStuff(inst)
    ...
    for i, v in ipairs(ents) do
        if v:IsValid() and not (v.components.inventoryitem.nobounce or inst.sg.mem.recentlybounced[v]) and v.Physics ~= nil and v.Physics:IsActive() then
            ...
            local intensity = math.clamp((6 - distsq) / 27 --[[(36 - 9)]], 0, 1)
...
local function GetPoundTarget(inst)
	local x, y, z = inst.Transform:GetWorldPosition()
    local closestPlayer = nil
    for i, v in ipairs(AllPlayers) do
        if not (v.replica.health:IsDead() or v:HasTag("playerghost")) and not v:HasTag("myth_yutu_underground") and
            v.entity:IsVisible() then
            local distsq = v:GetDistanceSqToPoint(x, y, z)
            if distsq < 16*16 then
                closestPlayer = v
            end
        end
    end
...
local function GetClosePlayer(inst)
	local x, y, z = inst.Transform:GetWorldPosition()
    local ClosestPlayer = nil
	local NumPlayer = 0
	local ents = TheSim:FindEntities(x, y, z, 8, {"player"},{"playerghost","myth_yutu_underground"})
    for i, v in ipairs(ents) do
		if  not (v.replica.health:IsDead() or v:HasTag("playerghost")) then
    ...
    return ClosestPlayer,NumPlayer
end

local function geiwoxialai(inst) --给我下来！
	local x, y, z = inst.Transform:GetWorldPosition()
	local ents = TheSim:FindEntities(x, 0, z, 8, {"player"}, {"playerghost", "FX", "NOCLICK"})
    for i, v in ipairs(ents) do
        if v:IsValid() and v.components.health
			and not v.components.health:IsDead() and  v.components.mk_flyer and v.components.mk_flyer:IsFlying() then
...
local function ChooseAttack(inst) --选择攻击方式
	if inst.smallfrog > 0 and not inst.components.timer:TimerExists("eat_cd") then
		local player,num = GetClosePlayer(inst)
		if  num > 1 and player ~= nil then
			inst.sg:GoToState("eatplayer",player)
			return true
		end
	end
...
local events =
{
    ...
    EventHandler("roar", function(inst) --咆哮
        if not (inst.sg:HasStateTag("busy") or inst.components.health:IsDead()) then
    ...
    EventHandler("gotospit", function(inst)
		if not (inst.sg.mem.gohome or inst.components.health:IsDead()) then
...
local states =
{
    ...
    State{
        name = "pound_pre",  
        tags = { "attack", "busy", "pounding", "nosleep", "nofreeze","noouthit" },
	...
	--TheInput:GetWorldEntityUnderMouse().sg:GoToState("pound_pre")
    State{
        name = "pound_pst",
        tags = { "attack", "busy", "pounding", "nosleep", "nofreeze","noouthit" },
        ...
        {
            EventHandler("animover", function(inst)
				local target = GetPoundTarget(inst)
                if inst.AnimState:AnimDone() then
					if inst.sg.mem.sleeping then
						inst.sg:GoToState("sleep")
					elseif target ~= nil  then
						inst.sg:GoToState("super_pound",target)
					else
						inst.sg:GoToState("idle")
					end
				end
    ...
    State{
        ...
        onenter = function(inst,target)
            inst.components.locomotor:StopMoving()
            inst.AnimState:PlayAnimation("attack_pound_pre")
			inst.AnimState:PushAnimation("attack_pound_loop")
			inst.AnimState:PushAnimation("attack_pound_pst",false)
            if target ~= nil then
        ...
        timeline =
        {
            ...
            TimeEvent(50 * FRAMES, function(inst)
				inst.components.groundpounder.groundpounddamagemult = 1.5
				inst.components.groundpounder:GroundPound()
				geiwoxialai(inst)
				DoPoundShake(inst)
				inst.SoundEmitter:PlaySound("dontstarve_DLC001/creatures/bearger/groundpound")
	...
	--TheInput:GetWorldEntityUnderMouse().sg:GoToState("eatplayer",ThePlayer)
    State{
        name = "eatplayer",
        ...
        timeline =
        {
            ...
            TimeEvent(20 * FRAMES, function(inst)
				...
				if target and target:IsValid() and target:HasTag("player") and not target.components.health:IsDead() then
					local x, y, z = inst.Transform:GetWorldPosition()
					local distsq = target:GetDistanceSqToPoint(x, y, z)
					if distsq < 49 then
						inst:OnEat(target)
						if target.components.mk_flyer ~= nil and  target.components.mk_flyer:IsFlying() then
							target.components.mk_flyer:SetFlying(false)
							target.components.mk_flyer._percent:set(0)			
						end
						inst.components.timer:StartTimer("on_eat_player", 10)
						inst.components.timer:StartTimer("eat_cd", 60)
					else
						inst.components.timer:StartTimer("eat_cd", 30)
					end
				else
					inst.components.timer:StartTimer("eat_cd", 30)
				end
...
return StateGraph("myth_goldfrog", states, events, "idle")
