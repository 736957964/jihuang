local assets =
{
    ...
    Asset("ANIM", "anim/myth_banana_tree.zip"),	
}

local leafassets =
{
    Asset("ANIM", "anim/myth_banana_leaf.zip"),
	Asset("ATLAS", "images/inventoryimages/myth_banana_leaf.xml"),
}
...
local function onpickedfn(inst,picker)
	if picker and picker.components.inventory then
		picker.components.inventory:GiveItem(SpawnPrefab("myth_banana_leaf"), nil, inst:GetPosition())
...
local function setupstump(inst)
    SpawnPrefab("myth_banana_stump").Transform:SetPosition(inst.Transform:GetWorldPosition())
    inst:Remove()
end
...
local function tree_chopped(inst, worker)
    ...
    if inst.components.pickable ~= nil and inst.components.pickable.canbepicked then
        ...
        inst.components.lootdropper:SpawnLootPrefab("myth_banana_leaf")
		inst.components.lootdropper:SpawnLootPrefab("myth_banana_leaf")		
    end
...
local function tree_burnt(inst)
    local burnt_tree = SpawnPrefab("myth_banana_burnt")
...
local function tree_fn()
    ...
    inst.MiniMapEntity:SetIcon("myth_banana_tree.tex")

    inst:AddTag("plant")

    inst.AnimState:SetBank("cave_banana_tree")
    inst.AnimState:SetBuild("myth_banana_tree")
    inst.AnimState:PlayAnimation("idle_loop", true)
...
local function stump_fn()
    ...
    inst:SetPrefabNameOverride("myth_banana_tree")
...
local function leaf_fn()
    ...
    inst.AnimState:SetBank("myth_banana_leaf")
    inst.AnimState:SetBuild("myth_banana_leaf")
	...
	inst.components.inventoryitem.atlasname = "images/inventoryimages/myth_banana_leaf.xml"
    ...
    inst.components.edible.healthvalue = 1.5*TUNING.HEALING_TINY/2
    inst.components.edible.hungervalue = 1.5*TUNING.CALORIES_TINY
...
return Prefab("myth_banana_tree", tree_fn, assets, prefabs_tree),
    Prefab("myth_banana_burnt", burnt_fn, assets, prefabs_burnt),
    Prefab("myth_banana_stump", stump_fn, assets, prefabs_stump),
	Prefab("myth_banana_leaf", leaf_fn, leafassets),
	MakePlacer("myth_banana_tree_placer", "cave_banana_tree", "myth_banana_tree", "idle_loop")
