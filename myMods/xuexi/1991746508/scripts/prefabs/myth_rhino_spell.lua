...
local NON_COLLAPSIBLE_TAGS = {"mythTag_rhino","flying", "bird", "ghost", "playerghost", "FX", "NOCLICK", "DECOR", "INLIMBO" }
lo = { "flying", "bird", "ghost", "locomotor", "FX", "NOCLICK", "DECOR", "INLIMBO" }
...
local function donextcollapse(inst)
    ...
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, 0, z, TUNING.ANTLION_SINKHOLE.RADIUS + 1, nil, NON_COLLAPSIBLE_TAGS, COLLAPSIBLE_TAGS)
    for i, v in ipairs(ents) do
        --if v:HasTag('mythTag_rhino') and not v:HasTag('rhino3_yellow') then
        --    v:OnAttackedBySpell()
        --end
        if v:IsValid() then
...
return Prefab("rhino3_spell_yellow", fn, assets, prefabs)
