...
local function MakeVeggie(name, has_seeds)
    local assets =
    {
		...
		Asset("ANIM", "anim/farm_plant_gourd.zip"),
        Asset( "ATLAS", "images/inventoryimages/"..name..".xml" ),
		Asset( "ATLAS", "images/inventoryimages/"..name.."_seeds.xml" ),
		Asset( "ATLAS", "images/inventoryimages/"..name.."_oversized.xml" ),
    }
	if MYVEGGIES[name].lure_data ~= nil then
		table.insert(assets, Asset("ANIM", "anim/"..MYVEGGIES[name].lure_data.build..".zip"))
	end
	if has_seeds then
	...
	local dryable = MYVEGGIES[name].dryable
    ...
    local function fn_seeds()
        ...
        inst.AnimState:PlayAnimation("seeds") --种子哦
    ...
    local function fn()
        ...
        local float = MYVEGGIES[name].float_settings
        if float ~= nil then
        ...
        inst.components.edible.secondaryfoodtype = MYVEGGIES[name].secondary_foodtype
		...
		if MYVEGGIES[name].lure_data ~= nil then
			inst:AddComponent("oceanfishingtackle")
			inst.components.oceanfishingtackle:SetupLure(MYVEGGIES[name].lure_data)
		end

		local halloweenmoonmutable_settings = MYVEGGIES[name].halloweenmoonmutable_settings
    ...
    local function fn_cooked()
        ...
        local float = MYVEGGIES[name].cooked_float_settings
        if float ~= nil then
            MakeInventoryFloatable(inst, float[1], float[2], float[3])
        else
            MakeInventoryFloatable(inst)
    ...
    local function fn_oversized()
        ...
        inst:AddComponent("perishable")
        inst.components.perishable:SetPerishTime(MYVEGGIES[name].perishtime * OVERSIZED_PERISHTIME_MULT)
        inst.components.perishable:StartPerishing()
        ...
        inst:AddComponent("inspectable")
        inst:AddComponent("inventoryitem")
		inst.components.inventoryitem.atlasname = "images/inventoryimages/"..name.."_oversized.xml"
