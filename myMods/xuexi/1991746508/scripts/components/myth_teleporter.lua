local function onavailable(self)
    if self.targetTeleporter ~= nil and self.enabled == true then
        self.inst:AddTag("myth_teleporter")
    else
        self.inst:RemoveTag("myth_teleporter")
    end
end
...
function Teleporter:SetTestFn(fn)
    self.teleportestfn = fn
end

function Teleporter:IsActive(doer)
	if self.banned and doer.components.myth_favorability and doer.components.myth_favorability:IsBanned() then
		return false
	end
	if self.teleportestfn  ~= nil then
		return self.teleportestfn(self.inst,doer) and self.enabled and self.targetTeleporter ~= nil
	end
...
function Teleporter:IsTargetBusy()
    return self.targetTeleporter ~= nil and self.targetTeleporter.components.myth_teleporter:IsBusy()
end
...
function Teleporter:Activate(doer)
    if not self:IsActive(doer) then
        return false
    end
    ...
    if self.targetTeleporter.components.myth_teleporter ~= nil then
        if self.targetTeleporter.components.myth_teleporter.onActivateByOther ~= nil then
            self.targetTeleporter.components.myth_teleporter.onActivateByOther(self.targetTeleporter, self.inst, doer)
        end
        self.targetTeleporter.components.myth_teleporter.numteleporting = self.targetTeleporter.components.myth_teleporter.numteleporting + 1
    end
    ...
    if self.targetTeleporter.components.myth_teleporter ~= nil then
        if doer:HasTag("player") then
            self.targetTeleporter.components.myth_teleporter:ReceivePlayer(doer)
        elseif doer.components.inventoryitem ~= nil then
            self.targetTeleporter.components.myth_teleporter:ReceiveItem(doer)
        end
    end
...
function Teleporter:Teleport(obj)
    if self.targetTeleporter ~= nil then
        ...
        local offset = self.targetTeleporter.components.myth_teleporter ~= nil and self.targetTeleporter.components.myth_teleporter.offset or 0
...
local function ondoerarrive(inst, self, doer)
    ...
    if not doer:IsValid() then
        doer = nil
    elseif doer.sg.statemem.myth_teleportarrivestate ~= nil then
        doer.sg:GoToState(doer.sg.statemem.myth_teleportarrivestate)
    end
    self.numteleporting = self.numteleporting - 1
...
function Teleporter:LoadPostPass(newents, savedata)
    if savedata ~= nil and savedata.target ~= nil then
        ...
        if targEnt ~= nil and targEnt.entity.components.myth_teleporter ~= nil then
